...
class loss:
    ...
    def I(self, y_output_u, y_output_v, ind, X, XV):
        ...
        xw = torch.cat(tuple([XV[i] for i in range(self.setup['dim'])]), dim=2)[:, ::self.setup['step'], :]
        ...
        for i in range(self.setup['dim']):
            du['du_' + str(i)] = X[i].grad[:, ::self.setup['step'], :].to(device)
        phi.backward(torch.ones_like(phi).to(device), retain_graph=True)
        dphi = {}
        for i in range(self.setup['dim'] + 1):
            dphi['dphi_' + str(i)] = XV[i].grad[:, ::self.setup['step'], :].to(device)
