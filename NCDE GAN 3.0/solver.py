...
def func_g(BX):
    return func_u_sol(BX)[:, ::setup['step']]
...
def func_a(xt, i, j):
    if i == j:
        return torch.ones(xt.shape[:-1])[:, ::setup['step']]
    else:
        return torch.zeros(xt.shape[:-1])[:, ::setup['step']]
...
class Comb_loader(Dataset):
    ...
    def __init__(self, intervals, step, batch_size, depth, border_batch_size, dim, down=-1, up=1, T0=0, T=1,
        ...
        self.dataset = FlexibleCDEDataset(self.txy, self.btxy, depth=depth)

        self.logsig_dim = self.dataset.logsig_dim
...
def funcs(points, setup):
    ...
    a = torch.Tensor(setup['dim'], setup['dim'], setup['batch_size'], setup['intervals'])
    ...
    b = torch.Tensor(setup['dim'], setup['batch_size'], setup['intervals'])
...
def L_norm(X, predu, p, T0=0, T=1):
    ...
    u_sol = func_u_sol(xt)[:, ::setup['step']].to(device).unsqueeze(2)
...
def train(params, checkpoint_dir=None):
    i = iter(params.items())
    config = dict(itertools.islice(i, 12))
    ...
    u_net = torch.nn.DataParallel(NeuralRDE(setup['dim']+1, points.logsig_dim, config['u_hidden_dim'], 1, hidden_hidden_dim=config['u_hidden_hidden_dim'], num_layers=config['u_layers'], return_sequences=True)).to(device)
    ...
    scheduler_u = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_u, factor=config['u_factor'], patience=5)
    ...
    for k in range(iterations):
        ...
        for i in range(n1):
            for ind, (data, bdata) in enumerate(dl):
                ...
                prediction_v = v_net(points.interiorv)[:, ::setup['step'], :]
