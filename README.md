# PDE-Solver
Our algorithm uses NODE networks in a GAN architecture to solve the following Parabolic PDE:
...
for a d-dimensional domain. It is able to handle both time-independent and time-varying domains, denoted by $\mathcal{D}$.

# Running Codes
Requirements for a successful implementation of the codes can be found in `requirements.txt`.

To solve a PDE one can input all the known functions of the problem in the file `main.py` which will run the algorithm. An example can be found in the `example.py` file in which the test problem reported in the paper is implemented.

In the `config` dictionary one can specify the hyperparameters required by the algorithm to solve the problem. 
He. Add the config dictionary list here.
...
Note that the data structure is `[N, L, 1+d]` where `N` is the number of points sampled in the special domain, `L` is the number of points sampled in the time regime,(sampled time points have to be the same for all spatial points) and `d` is the spacial dimension where. In the tensor, time is the top dimension (`[:, :, 0]` is the index for all times).
