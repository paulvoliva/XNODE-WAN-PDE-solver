...
assert npaths==4*bnpaths, "Need same size for dataloader to work"
...
for (initial_values, signatures), (binitial_values, bsignatures) in dataloader:
    print(initial_values.size())
    print(signatures.size())
    print(binitial_values.size())
    print(bsignatures.size())
...
def func_f(x, y, t):
    f = (math.pi ** 2 - 2) * torch.sin(math.pi / 2 * x) * torch.cos(math.pi / 2 * y) * torch.exp(
        -t/100) - 4 * torch.sin(math.pi / 2 * x) ** 2 * torch.cos(math.pi / 2 * y) ** 2 * torch.exp(-t/100)
    return(f)
