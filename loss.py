...
class loss:
    ...
    def __init__(self, alpha: float, a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, h: torch.Tensor, f: torch.Tensor,
                 g: torch.Tensor, setup: dict, domain, device):
        super(loss).__init__()
        self.T = setup['T']
        self.T0 = setup['T0']
        self.alpha = alpha
    ...
    def I(self, y_output_u: torch.Tensor, y_output_v: torch.Tensor, X: torch.Tensor, XV: torch.Tensor):
        ...
        Nt = y_output_u.shape[1]
        w = self.func_w(XV).unsqueeze(2).to(self.device)
        phi = y_output_v * w
        X.retain_grad()
        XV.retain_grad()
        y_output_u.backward(torch.ones_like(y_output_u).to(self.device), retain_graph=True)
        du = {}
        for i in range(self.setup['dim'] + 1):
            du['du_' + str(i)] = X.grad.to(self.device)[:, :, i]
        # TODO: special case of all 0 due to the same data structure? (worked before)
        phi.backward(torch.ones_like(phi).to(self.device), retain_graph=True)
        dphi = {}
        for i in range(self.setup['dim'] + 1):
            dphi['dphi_' + str(i)] = XV.grad.to(self.device)[:, :, i]
        s1 = self.V * (y_output_u[:, -1].squeeze() * y_output_v[:, -1].squeeze() - self.h * y_output_v[:, 0].squeeze()) / N
        s2 = self.V * (y_output_u.squeeze() * dphi['dphi_0']) / N / Nt
        s31 = [self.a[i, j] * dphi['dphi_' + str(i+1)] * du['du_' + str(j+1)] for i, j in
               product(range(self.setup['dim']), repeat=2)]
        s31 = torch.stack(s31, 0).sum(0)
        s32 = np.sum([self.b[i] * phi.squeeze() * du['du_' + str(i+1)] for i in range(self.setup['dim'])])
    ...
    def int(self, y_output_u: torch.Tensor, y_output_v: torch.Tensor, X: torch.Tensor, XV: torch.Tensor):
        ...
        N = y_output_v.shape[0] * y_output_v.shape[1]
        return torch.log(self.I(y_output_u, y_output_v, X, XV) ** 2) - torch.log(self.V * torch.sum(y_output_v ** 2) / N)
