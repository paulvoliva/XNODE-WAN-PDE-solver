...
def train(params, checkpoint_dir=None):
    ...
    u_net = NeuralODE(setup['dim'], config["u_hidden_dim"], 1, func_h, setup, hidden_hidden_dim=config['u_hidden_hidden_dim'], num_layers=config['u_layers'], return_sequences=True).to(device)   #torch.nn.DataParallel().to(device)
