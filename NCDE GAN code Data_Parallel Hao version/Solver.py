...
up = 1
down = -1
T = 1             #remember to divide this by 100 in all of the functions
T0 = 0
dim = 2
...
def func_u_sol(x, y, t):
    u = 2 * torch.sin(math.pi / 2 * x) * torch.cos(math.pi / 2 * y) * torch.exp(-t)
    return(u)

def func_f(x, y, t):
    f = (math.pi ** 2 - 2) * torch.sin(math.pi / 2 * x) * torch.cos(math.pi / 2 * y) * torch.exp(
        -t) - 4 * torch.sin(math.pi / 2 * x) ** 2 * torch.cos(math.pi / 2 * y) ** 2 * torch.exp(-2*t)
    return(f)
...
def L_bdry(u_net, border_logsig, ind, g=g):
    return torch.mean((u_net(border_logsig, btxy[ind*border_batch_size:(ind+1)*border_batch_size, :, :]) - g[ind*batch_size:(ind+1)*batch_size, :]) ** 2)
...
(initial, logsig), blogsig = next(iter(dataloader))
...
def train(config, checkpoint_dir=None):
    ...
    for k in range(iteration):
        ...
        batch, bbatch = next(dl)
        for i in range(n1):
            prediction_v = v_net(v_inp[ind*batch_size:(ind+1)*batch_size, :, :])
            prediction_u = u_net(batch, txy[ind*batch_size:(ind+1)*batch_size, :, :])
        ...
        if k % 10 == 0:
            ...
            error_test = torch.mean(
                torch.sqrt(torch.square((func_u_sol(x_setup[ind*batch_size:(ind+1)*batch_size, :, :], y_setup[ind*batch_size:(ind+1)*batch_size, :, :], t_setup[ind*batch_size:(ind+1)*batch_size, :, :]) - prediction_u.data)))).data
            ...
            datal = create_interval_dataloader(datas, sam, 2500)
            ...
            set1 = ax[0].contourf(mesh1.squeeze(1).data.numpy(), mesh2.squeeze(1).data.numpy(),
