...
x_domain_train = torch.Tensor(domain_sample_size, dim, t_mesh_size).uniform_(down, up)
...
x0_boundary_side = torch.Tensor(boundary_sample_size, dim - 1, t_mesh_size).uniform_(down, up)
...
x_boundary_train = torch.cat((x0_boundary_left, x0_boundary_right, x0_boundary_down, x0_boundary_up), 0)

xt_boundary_train = torch.cat((x_boundary_train, t[:4*boundary_sample_size, :, :]), dim=1) #TODO: Look at t and the dependencies (affecting grad?)
...
x_error = xu.clone().detach().to(device).requires_grad_(True)
y_error = yu.clone().detach().to(device).requires_grad_(True)
t_error = tu.clone().detach().to(device).requires_grad_(True)
...
h = func_h(xt_domain_train[:, :, 0].clone().detach().to(device)).to(device)
...
def L_init(y_output_u, ind, h=h):
    N = y_output_u.shape[0]
    return torch.log(torch.mean((y_output_u[:, 0, :].squeeze(1) - h[ind*N:(ind+1)*N]) ** 2))


def L_bdry(u_net, xt_boundary_train, ind, g=g):
    return torch.log(torch.mean((u_net(xt_boundary_train[:, 0, :].requires_grad_(True), xt_boundary_train[:, 1, :].requires_grad_(True), xt_boundary_train[:, 2, :].requires_grad_(True)) - g) ** 2))
    ...
    for k in range(iteration):
        ...
        for i in range(n1):
            for ind, (xu, yu, tu, xv, yv, tv, btxy) in enumerate(ds):
                xu, yu, tu, xv, yv, tv, btxy = xu.squeeze(0).to(device).requires_grad_(True), yu.squeeze(0).to(device).requires_grad_(True),\
                                               tu.squeeze(0).to(device).requires_grad_(True), xv.squeeze(0).to(device).requires_grad_(True),\
                                               yv.squeeze(0).to(device).requires_grad_(True), tv.squeeze(0).to(device).requires_grad_(True),\
                                               btxy.squeeze(0).to(device).requires_grad_(True)
                X = [xu, yu, tu]
                XV = [xv, yv, tv]
        ...
        if k % 10 == 0:
            ...
            x_mesh = torch.linspace(down, up, 500)
            mesh1, mesh2 = torch.meshgrid(x_mesh, x_mesh)
            ...
            dset = ax[1].contourf(mesh1.data.numpy(), mesh2.data.numpy(), net.squeeze(2).view(500,500).cpu().data.numpy(), 500, cmap='terrain')
            #eset = ax[2].contourf(mesh1.data.numpy(), mesh2.data.numpy(), mesh_1_.grad.view(500,500).data.numpy(), 500, cmap='winter')
            fset = ax[2].contourf(mesh1.squeeze(1).data.numpy(), mesh2.squeeze(1).data.numpy(), abs(func_u_sol(xt_test2.unsqueeze(2)).view(500,500).data.numpy()-net.squeeze(2).view(500,500).cpu().data.numpy()), 500, cmap='terrain')
            fig.colorbar(cset, ax=ax[0])
            ax[0].set_title('Correct Solution, Guess and Absolute Error at t=1')
