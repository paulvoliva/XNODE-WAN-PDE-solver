...
def L_norm(X: torch.Tensor, u_net, p: float, func_u_sol, volume: float, N_r: int):
    ...
    if not isinstance(X, list):
        return (volume * torch.mean(torch.pow(torch.abs(func_u_sol(X).to(X.device) - u_net(X.to(X.device)).squeeze()), p))) **(1/p)
    for x in X:
        rel = x.shape[0]/N_r
        diff += rel * torch.mean(torch.pow(torch.abs(func_u_sol(x).to(X.device) - u_net(x.to(X.device)).squeeze()), p))
...
def rel_err(X: torch.Tensor, predu: torch.Tensor, func_u_sol, p: float):
    ...
    '''
    u_sol = func_u_sol(X).to(X.device).unsqueeze(2)
