...
from NODE_GAN.utils.auxillary_funcs import proj, L_norm, rel_err
...
class NODE_WAN_solver:
    ...
    '''
    def __init__(self, params: dict, func_a, func_b, func_c, func_h, func_f, func_g, domain, func_u_sol=None,
                 p: float = 1):
        self.params = params
        self.func_a = func_a
        self.func_b = func_b
    ...
    def train(self, report: bool = False, report_it: int = 10, show_plt: bool = False):
        ...
        '''
        # TODO: remove rel json and print below
        print(self.u_net(0.1*torch.tensor([[[0.8944272398948669, 0.3, 0.3, 0.3, 0.3, 0.3], [1., 0.3, 0.3, 0.3, 0.3, 0.3]],[[0.8944272398948669, 0.3, 0.3, 0.3, 0.3, 0.3], [1., 0.3, 0.3, 0.3, 0.3, 0.3]],[[0.8944272398948669, 0.3, 0.3, 0.3, 0.3, 0.3], [1., 0.3, 0.3, 0.3, 0.3, 0.3]],[[0.8944272398948669, 0.4, 0.4, 0.4, 0.4, 0.4], [1., 0.4, 0.4, 0.4, 0.4, 0.4]]])))
        rel = []
        for k in range(self.iterations):
            domain = self.domain(self.setup['shape_param'], self.setup['dim'], self.setup['T0'], self.setup['T'],
                                 self.setup['N_t'])
            points = Comb_loader(self.setup['N_r'], self.setup['N_b'], domain)
            ...
            for i in range(self.n1):
                ...
                for (datau, datav, bdata) in points:
                    ...
                    # TODO: check that the model is the same at each step
                    ...
                    Loss = loss(self.config['alpha'], a, b, c, h, f, g, self.setup, domain)
                    # TODO: check loss
                    loss_u = Loss.u(prediction_u, prediction_v, self.u_net, datau, datav, bdata)
                    self.av_l += loss_u
                    loss_u.backward(retain_graph=True)
                self.optimizer_u.step()
                rel.append(rel_err(datau, prediction_u, self.func_u_sol, 1).item())
                with open("NLoss_rel_dim_"+str(self.setup['dim']) +".txt", "w") as fp:
                    json.dump(rel, fp)
            ...
            if report and k % report_it == 0:
                ...
                if self.func_u_sol is not None:
                    ...
                    #proj(self.u_net, self.setup, k, axes=[0, 1], resolution=200, colours=20, save=False,
