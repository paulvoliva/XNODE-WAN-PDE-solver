...
def fillt(inputs: torch.Tensor, setup: dict, min_steps: int = 5):
    ...
    '''
    times = inputs[0, :, 0]
    min_step_ = (setup['T'] - setup['T0']) / min_steps
    ...
    for k in range(idx.shape[0] - 1):
        n = math.ceil((times[idx[k]].item()-2*min_step_-out[-1].item())/min_step_) + 1
...
class NSphere_THourglass:
    '''
    this class samples points in a hypersphere of radius r*(1-t) for t<(T-T0)/2 and r*t for t>(T-T0)/2
    ...
    def interior(self, N_r: int):
        ...
        bound[i] = self.r*(1 - time_data)[i].double()
    ...
    def boundary(self, N_b: int):
        ...
        for t in self.times.numpy():
            n = int(N_b * (1 - t) ** self.dim) if t < (self.T-self.T0)/2 else int(N_b * t ** self.dim)
            points = self.surf(n) * (1 - t) if t < (self.T-self.T0)/2 else self.surf(n) * t
    ...
    def func_w(self, x: torch.Tensor):
        ...
        res[idx] = self.r * (1 - x[:, :, 0][idx]) - dist[idx]
...
class Comb_loader(Dataset):
    ...
    def __init__(self, N_r: int, N_b: int, shape):
        ...
        self.interiorv = [i.clone().detach().requires_grad_(True) for i in self.interioru]
    ...
    def __getitem__(self, idx):
        ...
        r = (data_u.to(self.device).requires_grad_(True), data_v.to(self.device).requires_grad_(True), boundary_.to(self.device).requires_grad_(True))
        return r
