...
def train(params, checkpoint_dir=None):
    ...
    u_net = NeuralRDE(points.input_dim, config["u_hidden_dim"], 1, func_h, hidden_hidden_dim=config['u_hidden_hidden_dim'], num_layers=config['u_layers'], return_sequences=True).to(device)    #torch.nn.DataParallel().to(device)
    ...
    for k in range(iterations):
        ...
        for i in range(n1):
             for ind, (data, bdata) in enumerate(dl):
                ...
                bdata = (bdata[0].to(device), bdata[1].to(device))
