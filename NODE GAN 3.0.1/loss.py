...
class loss:
    ...
    def I(self, y_output_u, y_output_v, ind, X, XV):
        ...
        xw = torch.cat(tuple([XV[i+1][ind*N:(ind+1)*N] for i in range(self.setup['dim'])]), dim=2)
        ...
        for i in range(self.setup['dim']):
            du['du_' + str(i)] = X[i].grad[ind*N:(ind+1)*N].to(device)
        phi.backward(torch.ones_like(phi).to(device), retain_graph=True)
        dphi = {}
        for i in range(self.setup['dim'] + 1):
            dphi['dphi_' + str(i)] = XV[i].grad[ind*N:(ind+1)*N].to(device)
    ...
    def bdry(self, ind, u_net, N, border_data):
        r = torch.mean((u_net(border_data) - self.g[ind * N:(ind + 1) * N].unsqueeze(2)) ** 2)
