...
class NeuralRDE(nn.Module):
    ...
    def __init__(self,
        ...
        # Initial to hidden
        self.initial_layers = nn.Sequential(*[nn.Linear(1, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim)])
    ...
    def forward(self, inputs):
        ...
        initial, vals = inputs
        timesteps = vals[0, :, 0]
        h_ = self.h(initial).unsqueeze(1)
        h0 = self.initial_layers(h_)

        # Perform the adjoint operation
        out = rdeint(
            timesteps, h0, self.func, method=self.solver, adjoint=self.adjoint, return_sequences=self.return_sequences
        )
...
class _NRDEFunc(nn.Module):
    ...
    def __init__(self, input_dim, logsig_dim, num_layers=1, hidden_dim=15):
        ...
        ]) if num_layers > 0 else nn.Linear(input_dim, input_dim-1)
